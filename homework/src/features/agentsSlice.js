...
export const countAgentsNumberWithProps = (state, selectedProps) => {
  if (selectedProps === "building" || selectedProps === "idle") {
    return state.agents.agents.filter((agent) => agent.status === selectedProps)
      .length;
  }
  return state.agents.agents.filter((agent) => agent.type === selectedProps)
    .length;
};
